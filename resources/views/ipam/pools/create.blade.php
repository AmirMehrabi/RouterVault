...
@push('navbar-breadcrumb')
    <x-ui.breadcrumb :items="[
        ...
        ['label' => 'Create Pool', 'current' => true],
    ]" />
@endpush
...
<div class="space-y-6" x-data="createPoolForm()">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Create IP Pool</h1>
            <p class="text-sm text-gray-500 mt-1">Configure a new IP address pool for your network</p>
        </div>
    </div>
    ...
    <form action="{{ route('ipam.pools.store') }}" method="POST" class="space-y-6">
        ...
        <div class="flex items-center justify-end gap-3 bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
            <a href="{{ route('ipam.pools.index') }}" class="inline-flex items-center gap-2 px-6 py-2.5 border border-gray-300 text-gray-700 text-sm font-medium rounded-lg hover:bg-gray-50 transition-colors">
                Cancel
            </a>
